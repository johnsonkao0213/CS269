...
# from models import build_dab_dino_deformable_detr, build_dino
...
def build_model_main(args):
    ...
    elif args.modelname.lower() == 'dn_dab_detr':
    ...
    # elif args.modelname.lower() == 'dino':
...
def build_teacher_model_main(args):
    ...
    if args.modelname.lower() == 'dn_dab_detr':
        ...
        model = build_dab_deformable_detr_deformable_encoder_only(args2, model_only=True)
    # elif args.modelname.lower() == 'dn_dab_dino_deformable_detr':
    #     model = build_dab_dino_deformable_detr(args2, model_only=True)
    # elif args.modelname.lower() == 'dino':
    #     model = build_dino(args2, model_only=True)
    return model
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(#  wo_class_error, [lr_scheduler=lr_scheduler, ]args=args
            model, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm, teacher=teacher,
                wo_class_error=wo_class_error, args=args, postprocessors=postprocessors, logger=(logger if args.save_log else None))
        if args.output_dir:
