...
def build(image_set, args):
    ...
    img_folder=paths["flir_rgb"]['train_img']
    ann_file=paths['flir_rgb']['train_anno']

    if image_set == "train":
        print("====load train dataset====")
        print(f'image folder: {paths["flir_rgb"]["train_img"]}')
        print(f'ann file: {paths["flir_rgb"]["train_anno"]}')
        img_folder = paths["flir_rgb"]['train_img'] 
        ann_file   = paths['flir_rgb']['train_anno']
        if args.cdod:
            source_domain="flir_rgb"
            target_domain="flir_thermal"
            return DADataset(
    ...
    if image_set == "test":
        ...
        print(f'image folder: {paths["flir_thermal"]["val_img"]}')
        print(f'ann file: {paths["flir_thermal"]["val_anno"]}')
        img_folder=paths["flir_thermal"]['val_img']
        ann_file=paths['flir_thermal']['val_anno']
        domain_flag = 1     # 1 for target domain

    if image_set == "val":
        print("====load validation dataset====")
        print(f'image folder: {paths["flir_rgb"]["val_img"]}')
        print(f'ann file: {paths["flir_rgb"]["val_anno"]}')
        img_folder=paths["flir_rgb"]['val_img']
        ann_file=paths['flir_rgb']['val_anno']
        domain_flag = -1    # -1 or 0 for source domain

    if image_set == "test2":
        print("====load test2 dataset====")
        print(f'image folder: {paths["flir_thermal"]["val_img"]}')
        print(f'ann file: {paths["flir_thermal"]["val_anno"]}')
        img_folder=paths["flir_rgb"]['val_img']
        ann_file=paths['flir_rgb']['val_anno']
...
def get_paths(root):
    ...
    return {
        'flir_rgb': {
            'train_img': root / 'FLIR_ADAS_v2/images_rgb_train',
            'train_anno': root / 'FLIR_ADAS_v2/annotations/images_rgb_train.json',
            'val_img': root / 'FLIR_ADAS_v2/images_rgb_val',
            'val_anno': root / 'FLIR_ADAS_v2/annotations/images_rgb_val.json',
        },
        'flir_thermal': {
            'train_img': root / 'FLIR_ADAS_v2/images_thermal_train',
            'train_anno': root / 'FLIR_ADAS_v2/annotations/images_thermal_train.json',
            'val_img': root / 'FLIR_ADAS_v2/images_thermal_val',
            'val_anno': root / 'FLIR_ADAS_v2/annotations/images_thermal_val.json',
        }
    }
